...
// -----------------------------------------------------------------------------
  ...
  My_RISCV_Core_ArbiterM5 u_output_arb (
  ...
  assign active_op1 = (addr_in_port == 1'b1) & !no_port;

  // addr_opess/control output decode
  assign i_hselm     = (addr_in_port == 1'b1) & sel_op1 & !no_port;
  assign HADDRM      = ( (addr_in_port == 1'b1) & !no_port ) ? addr_op1 : {32{1'b0}};
  assign HAUSERM     = ( (addr_in_port == 1'b1) & !no_port ) ? auser_op1 : {32{1'b0}};
  assign i_htransm   = ( (addr_in_port == 1'b1) & !no_port ) ? trans_op1 : 2'b00;
  assign HWRITEM     = (addr_in_port == 1'b1) & write_op1 & !no_port;
  assign HSIZEM      = ( (addr_in_port == 1'b1) & !no_port ) ? size_op1 : 3'b000;
  assign i_hburstm    = ( (addr_in_port == 1'b1) & !no_port ) ? burst_op1 : 3'b000;
  assign HPROTM      = ( (addr_in_port == 1'b1) & !no_port ) ? prot_op1  : {4{1'b0}};
  assign HMASTERM    = ( (addr_in_port == 1'b1) & !no_port ) ? master_op1 : 4'b0000;
  assign i_hmastlockm = (addr_in_port == 1'b1) & mastlock_op1 & !no_port;
  ...
  assign HWDATAM = ( data_in_port == 1'b1 ) ? wdata_op1 : {32{1'b0}};

  // HWUSERM output decode
  assign HWUSERM = ( data_in_port == 1'b1 ) ? wuser_op1 : {32{1'b0}};
