...
module My_RISCV_Core_ArbiterM0 (
...
// Signal declarations
// -----------------------------------------------------------------------------
    wire [0:0] addr_in_port_next; // D-input of addr_in_port
...
//  input port should be deselected.
  ...
  assign request = req_port0 | ( (iaddr_in_port == 1'b0) & HSELM &
                                (HTRANSM != 2'b00) );

  assign no_port_next     = ! ( HMASTLOCKM | request | HSELM );
  assign addr_in_port_next = ( request & !HMASTLOCKM ) ? 1'b0 : iaddr_in_port;
