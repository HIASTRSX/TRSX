...
// -----------------------------------------------------------------------------
    ...
    wire  [0:0] addr_in_port;   // addr_opess input port
...
// -----------------------------------------------------------------------------
  ...
  My_RISCV_Core_ArbiterM0 u_output_arb (
  ...
  assign active_op0 = (addr_in_port == 1'b0) & !no_port;

  // addr_opess/control output decode
  assign i_hselm     = (addr_in_port == 1'b0) & sel_op0 & !no_port;
  assign HADDRM      = ( (addr_in_port == 1'b0) & !no_port ) ? addr_op0 : {32{1'b0}};
  assign HAUSERM     = ( (addr_in_port == 1'b0) & !no_port ) ? auser_op0 : {32{1'b0}};
  assign i_htransm   = ( (addr_in_port == 1'b0) & !no_port ) ? trans_op0 : 2'b00;
  assign HWRITEM     = (addr_in_port == 1'b0) & write_op0 & !no_port;
  assign HSIZEM      = ( (addr_in_port == 1'b0) & !no_port ) ? size_op0 : 3'b000;
  assign i_hburstm    = ( (addr_in_port == 1'b0) & !no_port ) ? burst_op0 : 3'b000;
  assign HPROTM      = ( (addr_in_port == 1'b0) & !no_port ) ? prot_op0  : {4{1'b0}};
  assign HMASTERM    = ( (addr_in_port == 1'b0) & !no_port ) ? master_op0 : 4'b0000;
  assign i_hmastlockm = (addr_in_port == 1'b0) & mastlock_op0 & !no_port;
  ...
  assign HWDATAM = ( data_in_port == 1'b0 ) ? wdata_op0 : {32{1'b0}};

  // HWUSERM output decode
  assign HWUSERM = ( data_in_port == 1'b0 ) ? wuser_op0 : {32{1'b0}};
